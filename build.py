...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-audio {
    background: rgba(150,170,158,0.15); color: var(--accent);
    border: 1px solid rgba(150,170,158,0.3);
  }
  .btn-audio:hover { background: rgba(150,170,158,0.25); }
  .btn-doc {
    background: rgba(150,170,158,0.08); color: var(--subtext);
    border: 1px solid var(--border);
  }
  .btn-doc:hover { color: var(--accent); border-color: var(--accent2); }
  .btn-expand {
    background: transparent; color: var(--subtext);
    border: none; padding: 0; font-size: 0.75rem; cursor: pointer;
...
function doSearch() {
  ...
  if (semanticReady && embedFn) {
    embedFn(q).then(queryVec => {
      // Score each sermon by its best-matching chunk
...
function renderAll() {
  ...
  if (SERMONS.length === 0) {
    list.innerHTML = `<div class="empty-state"><strong>No sermons yet.</strong><p>Transcripts will appear here once processed.</p></div>`;
    return;
  }
  list.innerHTML = [...SERMONS]
    .sort((a, b) => b.date.localeCompare(a.date))
    .map(s => cardHtml(s, s.chunks[0], ''))
    .join('');
}

function renderResults(scored, q) {
  const list = document.getElementById('sermonList');
  updateCount(scored.length, SERMONS.length);
  if (scored.length === 0) {
    list.innerHTML = `<div class="empty-state"><strong>No results found.</strong><p>Try different keywords or a broader phrase.</p></div>`;
    return;
  }
  list.innerHTML = scored.map(({ sermon, chunk }) => cardHtml(sermon, chunk, q)).join('');
}
...
</html>""".replace('SERMONS_DATA_PLACEHOLDER', sermons_js)
